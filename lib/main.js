...
var compile = function(fileName, assets, S3, options, method, type, timestamp, callback) {
  ...
  return function(err, results) {
    if (err) {
      console.log(fileName, assets, options);
      throwError(err);
    }
    ...
    switch(method) {
      ...
      case 'minify':
        ...
        var minify = results;
        // Process images
        minify = minify.replace(/(background\-image|background|content)\:[^;]*url\((?!data:)['"]?([^\)'"]+)['"]?\)/ig, function (match, attribute, url) {
          if (options.production) {
        ...
        // Process fonts masqueraded as images
        // The Zocial CSS file, at least, has one entry that looks like this:
        // url('zocial-regular-webfont.svg#zocialregular') format('svg');
        minify = minify.replace(/url\((?!data:)['"]?([^\)'"]+\.(ttf|svg|eof|woff))(\#[a-z0-9]+)?['"]?\)/gi, function (match, url) {
          if (options.production) {
            var relativePath = url;
            if ('/' === relativePath[0]) {
              relativePath = relativePath.substr(1);
